...
try {
    ...
    // Consider both 'hospital' and 'hospital staff' or similar if necessary, simple matching for now
    users = allUsers.filter(u => u.relationship?.trim().toLowerCase() === 'hospital');
    console.log('Loaded Hospital users: ', users.length);
} catch (err) {
    console.error('Error loading CSV:', err);
}

test.describe('Hospital User Management Tests', () => {
    test.use({ storageState: 'storage/auth.ap_superadmin.json' });
    
    for (const user of users) {
        if (user?.name?.trim()) {
            test(`check Hospital User Registration - ${user.name}`, async ({ page }) => {
